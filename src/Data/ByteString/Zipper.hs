...
instance (NFData a) => NFData (ByteStringZipper a) where
...
getLineLimit :: ByteStringZipper a -> Maybe Int
getLineLimit = lineLimit

instance (Eq a) => Eq (ByteStringZipper a) where
...
instance (Show a) => Show (ByteStringZipper a) where
    show tz = concat [ "ByteStringZipper { "
                     , "above = "
                     , show $ above tz
                     , ", below = "
                     , show $ below tz
...
lineLengths :: (Monoid a) => ByteStringZipper a -> [Int]
...
-- text.
cursorPosition :: ByteStringZipper a -> (Int, Int)
...
isFirstLine :: ByteStringZipper a -> Bool
isFirstLine = null . above

isLastLine :: ByteStringZipper a -> Bool
isLastLine = (== 0) . length . below

nextLine :: ByteStringZipper a -> a
nextLine = head . below

-- |The line of text on which the zipper's cursor currently resides.
currentLine :: (Monoid a) => ByteStringZipper a -> a
...
-- the right.
insertChar :: (Monoid a) => Char -> ByteStringZipper a -> ByteStringZipper a
insertChar ch tz = maybe tz id $ insertChar_ ch tz

insertChar_ :: (Monoid a) => Char -> ByteStringZipper a -> Maybe (ByteStringZipper a)
insertChar_ ch tz
    | ch == '\n' = breakLine_ tz
...
breakLine_ :: (Monoid a) => ByteStringZipper a -> Maybe (ByteStringZipper a)
breakLine_ tz =
...
gotoEOL :: (Monoid a) => ByteStringZipper a -> ByteStringZipper a
...
killToEOL :: (Monoid a) => ByteStringZipper a -> ByteStringZipper a
killToEOL tz
...
-- current line.
killToBOL :: Monoid a => ByteStringZipper a -> ByteStringZipper a
...
deletePrevChar :: (Eq a, Monoid a) => ByteStringZipper a -> ByteStringZipper a
deletePrevChar tz
    | moveLeft tz == tz = tz
...
deleteChar :: (Monoid a) => ByteStringZipper a -> ByteStringZipper a
deleteChar tz
...
currentChar :: ByteStringZipper a -> Maybe Char
currentChar tz
  | not (null_ tz (toRight tz)) =
...
nextChar :: (Monoid a) => ByteStringZipper a -> Maybe Char
...
previousChar :: (Monoid a) => ByteStringZipper a -> Maybe Char
previousChar tz
...
gotoBOL :: (Monoid a) => ByteStringZipper a -> ByteStringZipper a
...
moveRight :: (Monoid a) => ByteStringZipper a -> ByteStringZipper a
moveRight tz
...
moveLeft :: (Monoid a) => ByteStringZipper a -> ByteStringZipper a
moveLeft tz
...
moveUp :: (Monoid a) => ByteStringZipper a -> ByteStringZipper a
moveUp tz
...
moveDown :: (Monoid a) => ByteStringZipper a -> ByteStringZipper a
moveDown tz
...
transposeChars :: (Monoid a) => ByteStringZipper a -> ByteStringZipper a
transposeChars tz
...
stringZipper :: [String] -> Maybe Int -> ByteStringZipper String
stringZipper =
    mkZipper (:[]) drop take length last init null lines id

-- |Construct a zipper from vectors of characters.
vectorZipper :: [V.Vector Char] -> Maybe Int -> ByteStringZipper (V.Vector Char)
vectorZipper =
    mkZipper V.singleton V.drop V.take V.length V.last V.init V.null V.vecLines V.toList

-- |Empty a zipper.
clearZipper :: (Monoid a) => ByteStringZipper a -> ByteStringZipper a
clearZipper tz =
    tz { toLeft = mempty
       , toRight = mempty
       , above = []
